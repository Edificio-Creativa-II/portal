...
def generar_portal_web(df_resumen, df_maestra, metricas, df_mora_mes, df_mora_hist, m_salud, 
                       ...
                       # --- NUEVO PARÁMETRO: Pasamos el DataFrame de los comunicados ---
                       df_comunicados=None,
                       # --- NUEVO PARÁMETRO: Para la lista acumulativa de PDFs ---
                       df_pdfs_publicar=None): # <--- ¡Mete esta línea aquí!
    import plotly.offline as opy
    import pandas as pd
    import re
    from datetime import datetime
...
# =============================================================================
# --- GENERACIÓN DE CONTENIDO PARA LA NUEVA PESTAÑA: COMUNICADOS ---
# =============================================================================
    import os  # Nos aseguramos de tener la librería para revisar archivos reales

    html_lista_comunicados_tab = ""
    ver_comunicados_tab = False # Por defecto oculto si no hay registros
    
    # 📁 CONFIGURAMOS LA REFEERENCIA DE LA CARPETA LOCAL (CON C MAYÚSCULA)
    CARPETA_COMUNICADOS = "Comunicados" # <--- ¡NUEVA VARIABLE!

    if df_pdfs_publicar is not None and not df_pdfs_publicar.empty:
        try:
            df_comun_clean = df_pdfs_publicar.copy()
            
            # Limpieza segura de las columnas del Excel
            df_comun_clean['Nombre del Documento'] = df_comun_clean['Nombre del Documento'].fillna("Documento sin título").astype(str).str.strip()
            df_comun_clean['Nombre Archivo PDF'] = df_comun_clean['Nombre Archivo PDF'].fillna("").astype(str).str.strip()
            df_comun_clean['Categoría'] = df_comun_clean['Categoría'].fillna("General").astype(str).str.strip()
            
            # Filtro de filas válidas
            df_comun_clean = df_comun_clean[df_comun_clean['Nombre del Documento'] != ""]
            
            if not df_comun_clean.empty:
                ver_comunicados_tab = True 
                html_lista_comunicados_tab += '<div class="row g-3">'
                
                for _, pdf_row in df_comun_clean.iterrows():
                    p_nombre = pdf_row['Nombre del Documento']
                    p_archivo = pdf_row['Nombre Archivo PDF']
                    p_fecha = str(pdf_row.get('Fecha', '-')).split(" ")[0]
                    p_cat = pdf_row['Categoría']
                    
                    # 🔍 AQUÍ SE CONSTRUYE LA RUTA RELATIVA HACIA LA CARPETA LOCAL
                    # Une "Comunicados" con el archivo (Ej: "Comunicados/archivo.pdf")
                    ruta_archivo_fisico = os.path.join(CARPETA_COMUNICADOS, p_archivo) if p_archivo else "" # <--- ¡NUEVA LÍNEA!
                    
                    # --- CONTROL DE SEGURIDAD EN PYTHON REVISANDO LA CARPETA ---
                    # Ahora os.path.exists revisa 'ruta_archivo_fisico' (dentro de la carpeta) en lugar de buscarlo suelto
                    existe_archivo_fisico = os.path.exists(ruta_archivo_fisico) if p_archivo else False # <--- ¡MODIFICADA!
                    
                    if p_archivo == "" or p_archivo.lower() == "nan":
                        # Caso A: El administrador no escribió ningún nombre de archivo en el Excel
                        boton_html = f"""
                        <button class="btn btn-sm btn-light border text-muted px-3" style="font-weight: 600; font-size: 0.8rem; border-radius: 4px; cursor: not-allowed;" disabled>
                            Sin Adjunto
                        </button>
                        """
                    elif not existe_archivo_fisico:
                        # Caso B: El archivo está referenciado en el Excel pero NO EXISTE dentro de la carpeta /Comunicados
                        # Modificamos el texto para indicarle al administrador dónde debe guardarlo
                        boton_html = f"""
                        <button class="btn btn-sm btn-warning text-dark px-3" style="font-weight: 600; font-size: 0.8rem; border-radius: 4px; cursor: not-allowed;" disabled title="¡Alerta Administrador: El archivo físico no está en la carpeta '{CARPETA_COMUNICADOS}'!">
                            ⚠️ Guardar en /{CARPETA_COMUNICADOS}
                        </button>
                        """
                    else:
                        # Caso C: Todo está perfecto. Apuntamos el enlace para que el navegador busque dentro de la subcarpeta en GitHub
                        boton_html = f"""
                        <a href="./{CARPETA_COMUNICADOS}/{p_archivo}" download="{p_archivo}" target="_blank" class="btn btn-sm text-white px-3" style="background-color: #8d6e63; font-weight: 600; font-size: 0.8rem; border-radius: 4px; white-space: nowrap;">
                            Ver Circular
                        </a>
                        """ # <--- ¡MODIFICADA LA RUTA DEL ENLACE CON LA CARPETA!
                    
                    # Armamos la tarjeta
                    html_lista_comunicados_tab += f"""
                    <div class="col-md-6 col-12">
                        <div class="card shadow-sm border-0 h-100" style="border-left: 4px solid #a8c69f !important; background-color: #ffffff;">
                            <div class="card-body d-flex align-items-center justify-content-between p-3">
                                <div class="d-flex align-items-center me-3">
                                    <div class="pe-2" style="font-size: 1.6rem; color: #8d6e63;">📄</div>
                                    <div>
                                        <h6 class="mb-1 text-dark fw-bold" style="font-size: 0.92rem; line-height: 1.3;">{p_nombre}</h6>
                                        <div class="text-muted d-flex align-items-center gap-2" style="font-size: 0.75rem;">
                                            <span>📅 {p_fecha}</span>
                                            <span class="mx-1">|</span>
                                            <span class="badge bg-light text-secondary border px-2 py-0.5" style="font-size: 0.7rem;">📌 {p_cat}</span>
                                        </div>
                                    </div>
                                </div>
                                {boton_html}
                    ...
                    """
                html_lista_comunicados_tab += '</div>'
        except Exception as e_pdf_html:
            html_lista_comunicados_tab = f""


# =============================================================================
# --- AUTOMATIZACIÓN DEL POP-UP DESDE EL EXCEL (PRODUCCIÓN REAL) ---
# =============================================================================
    html_modal_emergente = ""
    
    if df_comunicados is not None and not df_comunicados.empty:
        if len(df_comunicados) == 1 and "mensaje forzado" in str(df_comunicados.iloc[0].get('Detalle / Mensaje', '')).lower():
            html_modal_emergente = ""
        else:
            try:
                df_filtrado = df_comunicados.copy()
                df_filtrado['Título'] = df_filtrado['Título'].fillna("").astype(str).str.strip()
                df_filtrado['Detalle / Mensaje'] = df_filtrado['Detalle / Mensaje'].fillna("").astype(str).str.strip()
                
                condicion = (df_filtrado['Título'] != "") & (df_filtrado['Título'] != "nan") & \
                            (df_filtrado['Detalle / Mensaje'] != "") & (df_filtrado['Detalle / Mensaje'] != "nan")
                
                df_filtrado = df_filtrado[condicion]
                
                if not df_filtrado.empty:
                    ultimo_aviso = df_filtrado.iloc[-1]
                    aviso_titulo = str(ultimo_aviso['Título'])
                    aviso_texto = str(ultimo_aviso['Detalle / Mensaje'])
                    aviso_fecha = str(ultimo_aviso.get('Fecha', datetime.now().strftime('%d/%m/%Y'))).split(" ")[0]
                    aviso_cat = str(ultimo_aviso.get('Categoría', 'General')).strip()
                    
                    html_modal_emergente = f"""
                    <div class="modal-aviso-overlay" id="modalAvisoCopropietarios">
                        <div class="modal-aviso-contenedor">
                            <div class="modal-aviso-cabecera">
                                <h2>🔔 AVISO IMPORTANTE</h2>
                                <div class="modal-aviso-meta">📅 {aviso_fecha} | 📌 Categoría: {aviso_cat}</div>
                            </div>
                            <div class="modal-aviso-cuerpo">
                                <p class="fs-5"><strong>{aviso_titulo}</strong></p>
                                <p style="white-space: pre-line;">{aviso_texto}</p>
                            </div>
                            <div class="modal-aviso-pie">
                                <button class="btn-aviso-entendido" onclick="document.getElementById('modalAvisoCopropietarios').style.display='none'">Entendido</button>
                            </div>
                        </div>
                    </div>
                    """
            except Exception as e:
                print(f"Nota: No se pudo renderizar el aviso ({str(e)})")


# =============================================================================
# --- LOGICA DE ACTIVACIÓN UNIFICADA (UNA SOLA VEZ AL FINAL) ---
# =============================================================================
    estados = {
        "bal": ver_balance,
        "res": ver_resumen_fin,
    ...
    c_comun = "active" if primera_activa == "comun" else ""
    p_comun = "show active" if primera_activa == "comun" else ""
    ...
    <head>
        ...
        <style>
            ...
            /* --- ESTILOS MODERNOS EXCLUSIVOS DEL POP-UP --- */
            .modal-aviso-overlay {{
                position: fixed; top: 0; left: 0; width: 100%; height: 100%;
                background-color: rgba(0, 0, 0, 0.7); backdrop-filter: blur(4px);
                display: {'flex' if html_modal_emergente else 'none'}; 
                justify-content: center; align-items: center; z-index: 999999;
            }}
            .modal-aviso-contenedor {{
                background-color: #ffffff; width: 90%; max-width: 550px;
                border-radius: 14px; box-shadow: 0 15px 35px rgba(0,0,0,0.35);
                overflow: hidden; animation: zoomInAviso 0.3s ease-out;
            }}
            @keyframes zoomInAviso {{
                from {{ transform: scale(0.85); opacity: 0; }}
                to {{ transform: scale(1); opacity: 1; }}
            }}
            .modal-aviso-cabecera {{
                background: linear-gradient(135deg, #c0392b, #e74c3c);
                color: #ffffff; padding: 20px; text-align: center;
            }}
            .modal-aviso-cabecera h2 {{ margin: 0; font-size: 1.5rem; font-weight: 700; letter-spacing: 0.5px; }}
            .modal-aviso-meta {{ font-size: 0.82rem; color: #f9d5d5; margin-top: 5px; opacity: 0.9; }}
            .modal-aviso-cuerpo {{ padding: 25px; color: #2c3e50; line-height: 1.6; }}
            .modal-aviso-pie {{ padding: 15px 25px; background-color: #f8f9fa; text-align: right; border-top: 1px solid #eedede; }}
            .btn-aviso-entendido {{
                background-color: #e74c3c; color: #ffffff; border: none;
                padding: 9px 22px; font-size: 0.95rem; font-weight: bold;
                border-radius: 6px; cursor: pointer; transition: all 0.2s;
            }}
            .btn-aviso-entendido:hover {{ background-color: #c0392b; transform: translateY(-1px); }}
        </style>
    </head>
    <body>
        ...
        <div class="container">
            <!-- --- NAVBAR DE PESTAÑAS (MUESTRA LA NUEVA PESTAÑA SI TIENE CONTENIDO) --- -->
            <ul class="nav nav-tabs mb-4" id="portalTabs" role="tablist">
                ...
                {f'<li class="nav-item"><button class="nav-link {c_comun}" data-bs-toggle="tab" data-bs-target="#tab-comunicados">📢 Comunicados</button></li>' if ver_comunicados_tab else ""}
            </ul>
            ...
            <div class="tab-content">
                ...
                <!-- --- NUEVA PESTAÑA INDEPENDIENTE: COMUNICADOS Y CIRCULARES --- -->
                {f"""<div class="tab-pane fade {p_comun}" id="tab-comunicados">
                    <div class="alert alert-light border shadow-sm d-flex align-items-center py-2 px-3 mb-4">
                        <span class="me-2">📁</span>
                        <span style="color: #5d4037;">Repositorio Informativo y Circulares emitidas por la Administración</span>
                    </div>
                    <h5 class="section-title">📌 Historial de Comunicados Oficiales</h5>
                    {html_lista_comunicados_tab}
                </div>""" if ver_comunicados_tab else ""}
            </div>
        </div>
...
if df_ing is not None:
    ...
    with st.sidebar:
        ...
        # 3. EJECUCIÓN DE LA SUBIDA (Con inyección inteligente de avisos cortos para vecinos)
        if st.session_state.get('ejecutar_subida', False):
            try:
                with st.status("🚀 Publicando en servidor...", expanded=True) as status:
                    
                    # A. LECTURA INTERNA DE LA BITÁCORA DE COMUNICADOS
                    aviso_texto_portal = None
                    aviso_titulo_portal = ""
                    aviso_fecha_portal = ""
                    aviso_cat_portal = ""
                    
                    try:
                        ruta_comunicados = obtener_ruta_recurso("Gestión ECreativa II v1.0.xlsx")
                        df_comunicados = pd.read_excel(ruta_comunicados, sheet_name='ControlComunicados')
                        df_comunicados.columns = [str(c).strip() for c in df_comunicados.columns]
                        df_comunicados = df_comunicados.dropna(how='all')

                        # =========================================================================
                        st.session_state['df_comunicados'] = df_comunicados.copy()
                        globals()['df_comunicados'] = df_comunicados  # Doble seguro para el buscador multi-capa
                        # =========================================================================
                        
                        if not df_comunicados.empty:
                            try:
                                df_comunicados['Fecha_Dt'] = pd.to_datetime(df_comunicados['Fecha'], errors='coerce')
                                df_ordenado = df_comunicados.sort_values('Fecha_Dt', ascending=False)
                            except:
                                df_ordenado = df_comunicados.copy()

                            # Evaluamos si la última fila vigente es exclusivamente un aviso de texto corto (sin adjuntos)
                            mask_solo_texto = (
                                df_ordenado['Archivo Adjunto (Opcional)'].isna() | 
                                (df_ordenado['Archivo Adjunto (Opcional)'].astype(str).str.strip() == "") | 
                                (df_ordenado['Archivo Adjunto (Opcional)'].astype(str).str.lower() == "nan")
                            )
                            avisos_filtrados = df_ordenado[mask_solo_texto]

                            if not avisos_filtrados.empty:
                                ultimo_aviso = avisos_filtrados.iloc[0]
                                aviso_fecha_portal = str(ultimo_aviso['Fecha']).split(' ')[0] if pd.notna(ultimo_aviso['Fecha']) else "Sin Fecha"
                                aviso_titulo_portal = str(ultimo_aviso['Título']).strip() if pd.notna(ultimo_aviso['Título']) else "Comunicado Urgente"
                                aviso_cat_portal = str(ultimo_aviso['Categoría']).strip() if pd.notna(ultimo_aviso['Categoría']) else "General"
                                # Escapamos los saltos de línea para que JavaScript no se rompa al procesar el string
                                aviso_texto_portal = str(ultimo_aviso['Detalle / Mensaje']).strip().replace("\n", "\\n").replace('"', '\\"')
                    except Exception:
                        pass # Si no existe la pestaña, continúa la subida con normalidad sin inyectar pop-ups

                    # B. CONEXIÓN CON GITHUB Y CARGA LOCAL DEL ARCHIVO
                    TOKEN_GITHUB = "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" 
                    REPO_GITHUB = "Edificio-Creativa-II/portal"
                    URL_API = f"https://api.github.com/repos/{REPO_GITHUB}/contents/index.html"
                    headers = {"Authorization": f"token {TOKEN_GITHUB}", "Accept": "application/vnd.github.v3+json"}
                    ...
                    # --- INYECCIÓN DEL POP-UP JAVASCRIPT (Exclusivo en el navegador del vecino) ---
                    if aviso_texto_portal:
                        script_pop_up = f"""
                        <script>
                            window.addEventListener('DOMContentLoaded', (event) => {{
                                alert("🚨 AVISO IMPORTANTE A LA COMUNIDAD\\n\\n📅 Fecha: {aviso_fecha_portal}\\n📌 Categoría: {aviso_cat_portal}\\n📢 Título: {aviso_titulo_portal}\\n\\n{aviso_texto_portal}");
                            }});
                        </script>
                        </head>
                        """
                        # Reemplazamos la etiqueta de cierre del head para incrustar nuestro script de alerta automático
                        html_content = html_content.replace("</head>", script_pop_up)

                    # C. SUBIDA DE DATOS FINALES AL REPOSITORIO
                    res = requests.get(URL_API, headers=headers, timeout=10)
                    sha = res.json().get('sha') if res.status_code == 200 else None
...
# --- GENERACIÓN DEL REPORTE EXTERNO HTML ---
    ...
    # =========================================================================
    # NUEVO: CARGA DE LA PESTAÑA DE COMUNICADOS DESDE EL EXCEL PRINCIPAL
    # =========================================================================
    if 'ruta_archivo' in locals() or 'archivo_excel' in locals():
        try:
            # NOTA: Usamos el nombre exacto de tu pestaña 'ControlComunicados'
            df_comunicados = pd.read_excel(ruta_archivo, sheet_name='ControlComunicados')
            
            # Limpiamos filas completamente vacías por si acaso
            df_comunicados = df_comunicados.dropna(how='all') 
            
        except Exception as e:
            # Si falla, creamos el DataFrame con las columnas reales de tu Excel
            df_comunicados = pd.DataFrame(columns=['Fecha', 'Título', 'Categoría', 'Detalle / Mensaje', 'Archivo Adjunto (Opcional)'])
    else:
        df_comunicados = pd.DataFrame(columns=['Fecha', 'Título', 'Categoría', 'Detalle / Mensaje', 'Archivo Adjunto (Opcional)'])
...
try:
    ...
    # =========================================================================
    # RECONEXIÓN MULTI-HOJA (MÓDULOS INDEPENDIENTES: POP-UP Y PDFs) (TROZO 24)
    # =========================================================================
    df_comunicados_listo = pd.DataFrame()
    df_pdfs_listo = pd.DataFrame()  # <-- Nueva variable para el repositorio de archivos

    try:
        ruta_comunicados = obtener_ruta_recurso("Gestión ECreativa II v1.0.xlsx")
        
        # ---------------------------------------------------------------------
        # FLUJO 1: LEER AVISOS EMERGENTES (POP-UP)
        # ---------------------------------------------------------------------
        df_directo = pd.read_excel(ruta_comunicados, sheet_name='ControlComunicados')
        df_directo.columns = [str(c).strip() for c in df_directo.columns]
        df_directo = df_directo.dropna(how='all')
        
        # Formateamos la fecha a DD/MM/AAAA para la visualización visual
        if 'Fecha' in df_directo.columns:
            df_directo['Fecha'] = pd.to_datetime(df_directo['Fecha']).dt.strftime('%d/%m/%Y')
        
        if not df_directo.empty:
            df_comunicados_listo = df_directo
            st.session_state['df_comunicados'] = df_directo

    except Exception as e_lectura_com:
        # Si la hoja de comunicados falla o está vacía, continúa en silencio
        pass

    try:
        # ---------------------------------------------------------------------
        # FLUJO 2: LEER PUBLICACIONES DE ARCHIVOS PDF
        # ---------------------------------------------------------------------
        df_pdfs_directo = pd.read_excel(ruta_comunicados, sheet_name='ControlPDFs')
        df_pdfs_directo.columns = [str(c).strip() for c in df_pdfs_directo.columns]
        df_pdfs_listo = df_pdfs_directo.dropna(how='all')
        
        # Opcional: También formateamos la fecha de los PDFs para que se vea ordenada en la tabla
        if 'Fecha' in df_pdfs_listo.columns:
            df_pdfs_listo['Fecha'] = pd.to_datetime(df_pdfs_listo['Fecha']).dt.strftime('%d/%m/%Y')
            
        # Guardamos en la memoria de sesión de Streamlit para persistencia
        st.session_state['df_pdfs_publicar'] = df_pdfs_listo

    except Exception as e_lectura_pdf:
        # Si la nueva hoja de PDFs falla o no existe, la inicializamos vacía para que no rompa
        df_pdfs_listo = pd.DataFrame()

    # 3. CONTROL DE TRIGER AUTOMÁTICO EN EL HTML
    # =========================================================================
    # Creamos una variable condicional: Si el dataframe final está vacío, 
    # enviamos False o desactivamos la pestaña para que el HTML NO renderice el Pop-up.
    ver_comunicados_popup = False if df_comunicados_listo.empty else True
    # =========================================================================
    ...
    generar_portal_web(
        ...
        ver_balance=True,                                              # 19
        ver_resumen_fin=True,                                          # 20
        ver_morosidad=True,                                            # 21
        ver_control=True,                                              # 22
        ver_pdfs=True,                                                 # 23    
        # --- NUEVO PARÁMETRO INYECTADO AQUÍ ---
        df_comunicados=df_comunicados_listo,                           # 24 (Irá vacío si borraste los mensajes del Excel)
        df_pdfs_publicar=df_pdfs_listo                                 # 25 (Para el repositorio de la Pestaña)
    )

    # =========================================================================
    # --- AUTOMATIZACIÓN DE PUBLICACIÓN EN GITHUB (RAMA MAIN CONFIGURADA) ---
    # =========================================================================
    try:
        import subprocess

        print("🚀 [GitHub Sync] Iniciando sincronización automatizada...")

        # 1. AGREGAR ÚNICAMENTE EL PORTAL WEB (IGNORA EL EXE Y EL EXCEL)
        subprocess.run(["git", "add", "index.html"], check=True)

        # 2. Commit automático con fecha y hora
        mensaje_commit = f"Actualización automática portal - {fecha_actualizacion}"
        subprocess.run(["git", "commit", "-m", mensaje_commit], check=True)

        # 3. Subida directa apuntando a la rama 'main'
        subprocess.run(["git", "push", "origin", "main", "-f"], check=True)

        print("✅ ¡Portal index.html sincronizado con éxito!")
        st.sidebar.success(
            "🚀 ¡Portal index.html actualizado y publicado en Servidor!"
        )

    except subprocess.CalledProcessError as e_git:
        print(f"❌ [GitHub Sync] Error al ejecutar comandos de Git: {e_git}")
        st.sidebar.warning(
            "⚠️ Portal generado localmente. No se pudo subir automáticamente a GitHub. Revisa la consola."
        )
    except Exception as e_general_git:
        print(
            f"❌ [GitHub Sync] Error inesperado en automatización de Git: {e_general_git}"
        )
    # =========================================================================
